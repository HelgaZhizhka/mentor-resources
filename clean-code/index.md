...
Справочник для студентов и менторов RS School: примеры, объяснения и вопросы для проверки качества кода. Канонический материал хранится в `clean-code/`; копии в инструментах ревью обновляются из него.

## Быстрая навигация

| Ситуация | С чего начать |
| --- | --- |
| Студент на этапе TypeScript | [Основы](Clean-Code-Fundamental-Part1.md), [TypeScript](TypeScript.md), [SOLID и принципы проектирования](Clean-Code-Fundamental-Part5.md) |
| Самопроверка перед PR | [Общий чеклист](Check-List.md), затем разделы по найденным проблемам |
| Менторское ревью | Условия задания и [школьный процесс ревью](https://github.com/rolling-scopes-school/docs/blob/master/docs/ru/mentoring/pull-request-review-process.md), затем [чеклист](Check-List.md) |
| React-проект | [React](React.md) вместе с [TypeScript](TypeScript.md) и [SOLID](Clean-Code-Fundamental-Part5.md) |
| Вёрстка и интерфейс | [HTML](HTML.md), [CSS](CSS.md), [UI/UX](UI-UX.md) |

## Как использовать материалы

1. **Сначала откройте задание.** Его критерии, разрешённые инструменты и ограничения определяют обязательные требования. Этапы ревью, сроки и порядок оценки задаёт школьная документация по ссылке выше.
2. **Проверяйте смысл рекомендации.** Ошибка выполнения, нарушение контракта и риск потери данных требуют исправления. Размер функции, стиль экспорта или папка компонента — повод для обсуждения, если задание не устанавливает конкретное правило.
3. **Обосновывайте замечания.** Укажите место, последствие и способ проверки исправления. Метки «плохо/хорошо» иллюстрируют конкретную проблему; они не запрещают все альтернативные решения.
4. **Адаптируйте обучение.** Сохраняйте одинаковые критерии задания, но подбирайте объём обратной связи и подробность подсказок под студента. Чеклист не является самостоятельной шкалой штрафов.
5. **Применяйте SOLID на практике.** Все пять принципов входят в материал. Объясняйте ответственность, контракты и зависимости через реальные сценарии изменения, согласуя решение с KISS и YAGNI.

Примеры внутри глав часто показывают фрагмент приложения: типы, импорты или API проекта могут задаваться окружающим кодом. Перед переносом проверьте зависимости, обработку ошибок и требования своего задания. Избранные самостоятельные примеры помечены в исходном Markdown и проверяются компилятором и сценариями выполнения; это не означает, что все фрагменты являются готовыми приложениями.
...
Исправления вносите в `clean-code/`, затем синхронизируйте поставляемые копии по инструкции в CONTRIBUTING.md репозитория. Для нового правила описывайте контекст, причину и корректный пример; спорные предпочтения не оформляйте как безусловные запреты.
