...
Перед преобразованиями зафиксируйте контракт и тесты: [безопасный рефакторинг](Safe-Refactoring.md).
...
- Функция больше 20–30 строк — сигнал проверить ответственность, а не ошибка сама по себе
...
- Позиционные параметры трудно различить и легко перепутать; более трёх — повод проверить читаемость вызовов
...
**Проблема:** Доменные значения представлены примитивами так, что их легко перепутать, а правила проверки дублируются. Примитивы сами по себе не являются ошибкой. Следующий пример сначала решает более простую проблему — длинный список аргументов.
...
Объект параметров группирует связанные аргументы. Это ещё не демонстрация Value Object: для него определяют семантику значения, например сравнение двух денежных сумм по валюте и величине. TypeScript-интерфейс сам по себе не задаёт сравнение и runtime-валидацию. Подробнее: [Value Object — Martin Fowler](https://martinfowler.com/bliki/ValueObject.html).
...
**Допустимо — switch с явным значением по умолчанию:**
...
**Альтернатива — Map для простого соответствия ключ → значение:**

```typescript
const ANIMAL_SOUNDS = new Map<string, string>([
  ['dog', 'woof'],
  ['cat', 'meow'],
  ['cow', 'moo'],
]);

const getAnimalSound = (animal: string): string => ANIMAL_SOUNDS.get(animal) ?? 'unknown';
```

`Map` не подставляет унаследованные свойства объекта для ключей вроде `toString` или `__proto__`. Проверьте такие входы при замене `switch`. Для фиксированного соответствия классы обычно не нужны.

**Полиморфизм — если варианты имеют собственное поведение:**
...
- Пробелы или табы — по настройке форматтера проекта; не смешивайте стили
  **Длина строки:**
- **80–120 символов** — ориентир для читаемости; конкретную ширину и исключения задаёт проект
