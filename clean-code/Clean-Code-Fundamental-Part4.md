...
// Учебная арифметика: представление денежных сумм и округление разобраны ниже.
...
2. **Один тест — один сценарий или аспект контракта**: Несколько assertions допустимы, если вместе проверяют одну концепцию
...
5. **Подмены зависимостей в unit-тестах**: Изолируйте проверяемую логику там, где это полезно. Отдельные интеграционные и контрактные тесты проверяют реальные границы: сериализацию, ошибки и поведение адаптеров внешних API. Мок не доказывает совместимость с сервисом.

О принципах FIRST, граничных случаях и сохранении поведения при изменениях — в [материале о безопасном рефакторинге](Safe-Refactoring.md).

### Денежные суммы и точность

Примеры цен выше показывают разделение вычислений и зависимостей. Для реальных денег нельзя безоговорочно складывать десятичные дроби в `number`: `0.1 + 0.2 !== 0.3`.

Храните суммы в минимальных единицах валюты (например, копейках) в пределах безопасного целочисленного диапазона либо используйте десятичную арифметику. Явно задайте валюту, точность, момент и правило округления налогов и скидок. Проверяйте дробные суммы и границы округления, а не только целые цены. `number` пригоден для безопасных целых; дело в представлении и операциях, а не в запрете типа.
