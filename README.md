...
#### Pocket Mentor или Student Reviewer — что выбрать?

Это **два разных инструмента с разными задачами**. Они не заменяют друг друга — они дополняют разные этапы работы со студенческим кодом.

|                          | **Pocket Mentor** (skill)                                | **Student Reviewer** (GitHub Action)                           |
| ------------------------ | -------------------------------------------------------- | -------------------------------------------------------------- |
| **Когда запускается**    | Ментор вручную перед оценкой                             | Автоматически на каждый push в PR                              |
| **Кто читает результат** | Ментор → курирует → решает что отправить студенту        | Студент напрямую                                               |
| **Что анализирует**      | Весь проект целиком (структура, фичи, баллы по ТЗ)       | Только diff текущего PR (качество написанного кода)            |
| **Роль**                 | Помощник ментора для финальной оценки                    | Образовательный feedback студенту во время разработки          |
| **Вывод**                | `CODE_REVIEW_REPORT.md` со Score + опционально PR/Issues | Inline-комментарии в PR с объяснением концепций                |
| **Стек**                 | HTML/CSS, Vanilla JS, TypeScript, React+TS               | HTML/CSS, Vanilla JS, TypeScript, React+TS (Angular — не подд.) |
...
- **Идеальный сценарий** — оба: студент во время разработки исправляет проблемы качества кода через action, ментор перед финальной оценкой запускает pocket-mentor для проверки структуры и выставления баллов.
