...
## 2026-09-09 — Clean-code curriculum corrections

**Done:**
- Corrected unknown narrowing, external API validation, debounce inference, floating-Promise guidance, React memo comparison, incomplete form fields, and invalid HTML nesting.
- Clarified contextual guidance for function/file sizes, parameters, mutation, component layout/exports, CSS selectors and responsive styles; aligned review checklists.
- Preserved SOLID as a full core section, added review questions for all five principles and replaced the hook factory with a narrow dependency-injection example. Added error/late-response handling to the separation-of-concerns hook fragment.
- Rebuilt the curriculum index from existing sections, removed stale anchors, and added student/mentor entry points with source ownership and task-first review guidance.
- Added scripts/verify-clean-code-examples.mjs: compiles six actual Markdown snippets in strict TypeScript and checks their behavior. Documented usage and limits in CONTRIBUTING.md.
- Synced canonical curriculum into both skills and the student-reviewer action. Verified six examples, local curriculum links/anchors, byte-identical reference copies, git diff whitespace and ./init.sh.

**Decisions:**
- SOLID stays a full part of the curriculum, not optional reading deferred outside the core material.
- Curriculum is edited only in clean-code/; bundled reference copies are generated by sync scripts. School docs own review process and scoring; assignments own explicit requirements.
- No skill prompts or checker behavior changed; no skill version changes or tool removals. The six-example check is not a full build of all snippets or a React application test.

**Next:**
- Review this curriculum change and prepare the separate school-docs update replacing deleted-page links and obsolete automation instructions. Do not restore deleted pages.
- Decide the future of skills/agents separately; any continuation of automated-review work should use a concrete TS-stage mentor assignment.

**Blockers:** None for this curriculum iteration. School-docs changes remain separate work.
