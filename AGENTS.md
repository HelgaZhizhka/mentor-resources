...
5. Commit with a descriptive message — commit messages **are** the progress log.
...
- **Skill changes are versioned.** Use semantic intent: bug fix → patch, new checker/flag → minor.
- **No `progress.md` or `feature_list.json`.** Git history is the session log; commit messages must be informative enough to reconstruct intent.
...
2. Commit any clean-state changes with a descriptive message.
3. If you leave anything uncommitted on purpose, write a brief note in the WIP commit message explaining why.
